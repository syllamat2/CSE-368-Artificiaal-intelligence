...
class Solver:
    def AC3(self, csp, queue=None, removals=None):
        ...
        while queue:
            (i, j) = queue.pop(0)
            if Solver.revise(self, csp, i, j, removals):
                if len(csp.curr_domains[i]) == 0:
                    return False
    ...
    def backtrack(self, assignment, csp):
        ...
        var = Solver.select_unassigned_variable(self, assignment, csp)
        for value in Solver.order_domain_values(self, var, assignment, csp):
            if csp.nconflicts(var, value, assignment) == 0:
                assignment[var] = value
                sup = csp.suppose(var, value)
                interf = csp.infer_assignment()
                if interf is not None:
                    result = Solver.backtrack(self, assignment, csp)
